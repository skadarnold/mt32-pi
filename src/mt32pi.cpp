	...
	if (m_pLuaState)
	{
		...
		else
		{
			...
			if (lua_pcall(m_pLuaState, 3, LUA_MULTRET, 0) != LUA_OK)
			{
				...
				CLogger::Get()->Write(MT32PiName, LogError, pFormat, pMessage);
...
void CMT32Pi::UpdateNetwork()
{
	...
	if (!m_bNetworkReady && bNetIsRunning)
	{
		...
		if (m_pConfig->NetworkLuaREPL && m_pLuaState)
		{
			...
			{
				m_pLogger->Write(MT32PiName, LogError, "Failed to init Lua REPL");
				delete m_pLuaREPL;
				m_pLuaREPL = nullptr;
			}
			else
				m_pLogger->Write(MT32PiName, LogNotice, "Lua REPL initialized");
		}
	}
